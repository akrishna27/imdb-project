...
//	printf("the last input is %s\n", gamma);
	...
	int assignto=1;	//variable to determine where values are assigned to	
	int loop=0;
	struct film* x=NULL;
	int valueadded=1;	//variable showing whether or not last value was added
	
	
	while(scanf("%s\n",curr)!=EOF){		//gets one line of input to parse through
		printf("current line is %s\n",curr);
		if(loop==0){		//skip categories
				loop++;
				continue;
		}
		...
		if(loop==10){
			printf("ten loops done\n");
				break;
		}
		x=(struct film*)malloc(sizeof(film));	//current film record being made
		
		char* val=strtok(curr,",");		//accept token
		
		
		while(val!=NULL){
			
			if(strcmp(val,"")==0){	//empty value sequence
				if(assignto==1){
					x->color="";
				}else if(assignto==2){
					x->director_name="";
				...
				assignto++;		//move on to next value in sequence
				valueadded=1;
			}					//end of empty string case sequence
			
			if((assignto==3)|(assignto==4)|(assignto==5)|(assignto==6)|(assignto==8)|(assignto==9)|(assignto==13)|(assignto==14)|(assignto==16)|(assignto==19)|(assignto==23)|(assignto==24)|(assignto==25)|(assignto==28)){	//convert to int
				int numval=atoi(val);
				if(assignto==3){
					x->num_critic_for_reviews=numval;
				}else if(assignto==4){
				...
				assignto++;
				valueadded=1;
			}			//end of int entry case
			
			if((assignto==26)|(assignto==27)){		//convert to double
				if(assignto==26){
					x->imdb_score=atof(val);
				}else{
					x->aspect_ratio=atof(val);
			...
			if(assignto==1){
				...
				if(valueadded==1){	//if next/first in sequence, value gets added
					x->color=val;
				}else{				//if last entry was unfinished, concats to previous entry, adding space in between
					char* space=" ";
					char* temp=x->color;
					strcat(temp,space);
					strcat(temp,val);
				...
				if(valueadded==1){
					...
					char* space=" ";
					char* temp=x->director_name;
					strcat(temp,space);
					strcat(temp,val);
					x->director_name=temp;
				}
				assignto++;
			}else if(assignto==7){
				...
				if(valueadded==1){
					...
					char* space=" ";
					char* temp=x->plot_keywords;
					strcat(temp,space);
					strcat(temp,val);
					x->plot_keywords=temp;
				}
				assignto++;
			}else if(assignto==18){
				if(valueadded==1){
					...
					char* space=" ";
					char* temp=x->movie_imdb_link;
					strcat(temp,space);
					strcat(temp,val);
					x->movie_imdb_link=temp;
				}
				assignto++;
			}else if(assignto==20){
				if(valueadded==1){
				...
				if(valueadded==1){
					...
					char* space=" ";
					char* temp=x->content_rating;
					strcat(temp,space);
					strcat(temp,val);
					x->content_rating=temp;
				}
				assignto++;
			}	
		...
		if (assignto!=28){		//unfinished entry for last value
				valueadded=0;
		}else{					//finished film value, store film then reset assignto variable
			//store x in array
			assignto=1;
		}
